package cn.ytang.james.recyclerviewlib.flexibledivider;
    ...
    for (int i = 0; i < itemCount; i++) {
      ...
      if (child != null) {
        ...
        if (childPosition >= builder.spanCount) {
          ...
          final int right1 = child.getRight() + params.rightMargin
              + Math.round(ViewCompat.getTranslationX(child) + builder.h_spacing);
          final int top1 = child.getTop() + params.topMargin
              + Math.round(ViewCompat.getTranslationY(child)) - builder.v_spacing;
